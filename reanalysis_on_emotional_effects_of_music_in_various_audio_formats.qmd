...
f3dlisten <- sociodemographics_filtered %>%
  filter(familiarity_3d_audio == "listened to music in 3d audio") %>% nrow()
```
`r music_prof` out of `r N` participants
(`r round(music_prof / N * 100, digits = 1)`%) reported a music-related
profession or course of study.
```{r music-related profession or course of study}
sociodemographics_filtered %>% filter(music_related_profession_study) %>%
  select(occupation) %>% table()
```
Most of the participants stated that they listened to classical music regularly
(*n* = `r classical_reg`, `r round(classical_reg / N * 100, digits = 1)`%) or
occasionally (*n* = `r classical_occ`,
`r round(classical_occ / N * 100, digits = 1)`%).
`r stim_familiar` participants (`r round(stim_familiar / N * 100, digits = 1)`%)
were familiar with piece used as stimulus material and `r stim_familiar_well`
(`r round(stim_familiar_well / N * 100, digits = 1)`%) of these reported knowing
it very well. 
Regarding 3D audio, `r f3d` participants (`r round(f3d / N * 100, digits = 1)`%)
indicated that they heard or read about it, and another `r f3dlisten`
(`r round(f3dlisten / N * 100, digits = 1)`%) that they had already listened to
music in a 3D audio format.
...
#| warning: false
#| label: tbl-means
#| tbl-cap: Means, Standard Deviations, and Confidence Intervals for the Nine GEMS factors for the Three Audio Formats. Confidence intervals are calculated according to @cousineau_error_2014.
descriptives <- data_long_single %>%
  group_by(factor, format) %>%
  summarise(M = mean(score), SD = sd(score))
source("within_ci_function.R")
cis <- purrr::map(gems_factors,
                  function(gems) {
                    within_ci(data = data_long_mult,
                              subject = "participant",
                              condition = "format",
                              score = gems,
                              alpha = 0.05) %>% 
                      mutate(variable = variable %>% gsub("_", " ", .) %>%
                               tools::toTitleCase()) %>%
                      rename(factor = variable) %>%
                      select(factor, format, CI_LL, CI_UL)
                  }) %>%
  bind_rows() %>% group_by(factor)
descriptives <- descriptives %>%
  full_join(cis, by = join_by(factor, format))
descriptives %>% 
...
#| fig-width: 10
#| fig-cap: Means for the Nine GEMS Factors for the Three Audio Formats. Error bars represent 95% confidence intervals according to @cousineau_error_2014. Emotions felt are reported on a 5-point Likert scale.
ggplot(data = descriptives %>% group_by(format),
...
According to Mardia's test, multivariate normality is only present for surround
sound.
There are `r length(mv_outliers)` participants who are multivariate outliers at
least in one condition.
Excluding these participants would shrink the sample to
`r round((1 - length(mv_outliers) / N) * 100, digits = 1)`%.
Therefore, a non-parametric MANOVA will be used.
