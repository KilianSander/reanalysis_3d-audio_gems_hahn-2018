...
sapply(c("tidyverse", "MANOVA.RM", "rstatix", "ggpubr", "afex", "emmeans", "ggbeeswarm"),
...
transcendence_contrasts %>% select(-c(format1, format2, starts_with("sd"))) %>%
  knitr::kable()
```
```{r, transcendence contrasts}
transcendence_anova_contrasts <-
  contrast(emmeans(aov_car(transcendence ~ format + Error(participant/format),
                           data_long_mult, include_aov = TRUE,
                           emmeans_model = "univariate"),
                   ~ format),
           method = "revpairwise", adjust = "holm") # adjust = "none" to match JASP results
cbind(confint(transcendence_anova_contrasts, level = .95),
      transcendence_anova_contrasts %>% as.data.frame %>% select(c(t.ratio, p.value)))

eff_size(emmeans(transcendence_anova_afex, ~ format),
         edf = df.residual(transcendence_anova_afex$lm),
         sigma = sqrt(mean(sigma(transcendence_anova_afex$lm)^2)),
         level = 1 - .05/3) # matching JASP results
```
...
```
```{r cle table}
cle_table <- data.frame(format1 = c("Auro-3D", "Auro-3D", "Surround"),
