---
title: "Intensified Emotions due to 3D Audio? Comparative Reanalysis of a Data set on the Emotional Listening Experience in Stereo, 5.1 Surround Sound and 3D Audio"
...
```{r imei correlation matrix}
imei_cor_mat <- imei_data_wide %>%
  select(starts_with("mean_")) %>% cor_mat()
imei_cor_mat %>% cor_gather() %>% filter(var1 != var2) %>% relocate(var2) %>%
  filter(var2 != "mean_3d_imei", var1 != "mean_mono_imei") %>%
  mutate(var2 = recode(.$var2, mean_mono_imei = "Mono",
                       mean_stereo_imei = "Stereo"),
         var1 = recode(.$var1, mean_stereo_imei = "Stereo",
                       mean_3d_imei = "3D"),
         p = sapply(.$p, function(P) ifelse(P < .001, "< .001", P)),
         `Fisher z` = atanh(cor)) %>%
  knitr::kable(col.names = c("", "", "$r$", "$p$", "Fisher $z$"),
               align = "llccc")
```
