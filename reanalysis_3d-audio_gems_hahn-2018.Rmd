...
$$
Z_{\textrm{CLE}} =
\frac{M_1 - M_2}{\sqrt{SD_1^2+SD_2^2-2\cdot{}SD_1\cdot{}SD_2\cdot{}r_{12}}}
$$
```{r cle function}
get_cle <- function(mean1, mean2, sd1, sd2, r12) {
  Z_cle <- (mean1 - mean2) / sqrt(sd1^2 + sd2^2 - 2 * sd1 * sd2 * r12)
  Z_cle %>% pnorm(lower.tail = F)
}
```
```{r cle table}
cle_table <- data.frame(format1 = c("Stereo", "Stereo", "Surround"),
                        format2 = c("Surround", "Auro", "Auro"))
cle_table$cle <-
  mapply(function(f1, f2) {
    ...
    get_cle(mean1 = desc$M[desc$format == f1],
            ...
            r12 = cor_table$r[cor_table$Formats == paste(f1, f2,
                                                         sep = " -- ")])
  },
